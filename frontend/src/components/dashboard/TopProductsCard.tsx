        ...
        {products.map((product, idx) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className={cn('flex h-10 w-10 items-center justify-center rounded-xl text-lg', product.accentClass)}>
                {product.icon}
              </div>
              <div>
                <div className="text-sm font-semibold">{product.name}</div>
                <div className={cn('flex gap-3 text-[11px]', isDark ? 'text-slate-300' : 'text-slate-500')}>
                  <span>{product.sales}</span>
                  <span className="text-slate-500">{product.stock}</span>
                </div>
              </div>
            </div>
            <span className={cn('text-[11px] font-semibold', isDark ? 'text-emerald-200' : 'text-emerald-600')}>
              ● Available
            </span>
          </div>
        ))}
